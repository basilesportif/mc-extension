...
wit_bindgen::generate!({
    path: "target/wit",
    world: "gamelord-test-template-dot-os-v0",
...
fn handle_message (our: &Address) -> anyhow::Result<()> {
    ...
    print_to_terminal(0, "gamelord_test: a");
    ...
    let our_chat_address = Address {
        node: our.node.clone(),
        process: ProcessId::new(Some("gamelord"), "gamelord", "template.os"),
    };
    ...
    // Send
    print_to_terminal(0, "gamelord_test: b");
    ...
    print_to_terminal(0, "gamelord_test: c");
    let response = Request::new()
        .target(their_chat_address.clone())
        .body(ChatRequest::History(our.node.clone()))
        .send_and_await_response(15)?.unwrap();
    if response.is_request() { fail!("gamelord_test"); };
...
fn init(our: Address) {
    ...
    loop {
        match handle_message(&our) {
            Ok(()) => {},
            Err(e) => {
                print_to_terminal(0, format!("gamelord_test: error: {e:?}").as_str());
